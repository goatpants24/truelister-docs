...
## 2026-08-27 - [Line-Bounded Regex Extraction for OCR Text]
**Learning:** Using generic whitespace character classes (`\s`, which matches `\n` and `\r`) inside repeated capturing groups (`/made\s+in\s+([A-Za-z\s]+)/i`) causes regex engines to search across multiple lines during OCR text parsing. When matching fails downstream on multi-line text, this leads to catastrophic backtracking and line bleed across fields. Restricting the capturing class to horizontal whitespace (`[\t ]`) bounds the match strictly to the current line, eliminating backtracking overhead and preventing cross-line data corruption.
**Action:** In multi-line text parsers (such as OCR output), restrict string-capturing character classes to horizontal whitespace (`[\t ]`) instead of `\s` to prevent cross-line backtracking and field bleed.
