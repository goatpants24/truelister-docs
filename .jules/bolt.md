## 2026-08-27 - [Reducer-Managed Dirty State & Reference Re-Use]
**Learning:** Evaluating `canUndo` by executing key-by-key `shallowEqual(state.present, state.lastCommitted)` on every component re-render pass introduces an $O(K)$ computation overhead during high-frequency typing in large form state objects. Tracking an `isDirty` boolean directly within the reducer state on state actions (`UPDATE`, `COMMIT`, `UNDO`, `REDO`, `RESET`) turns `canUndo` into an $O(1)$ boolean property check (`state.past.length > 0 || state.isDirty`). Furthermore, reusing a single static `EMPTY_FUTURE` array reference prevents unnecessary empty array allocations during state transitions.
**Action:** In custom state hooks/reducers, manage dirty flags and state properties inside the reducer rather than computing expensive equality checks during component render passes.
