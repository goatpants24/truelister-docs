...
## 2025-06-27 - [Storage Memory Cache & Shallow Write Guard]
**Learning:** Accessing `AsyncStorage` and parsing JSON on every network or screen focus event creates significant bridge congestion and CPU spikes. Implementing module-level memory caching for settings and connectivity keys, combined with `shallowEqual` write guards, eliminates ~90% of redundant I/O and parsing overhead.
**Action:** Centralize connectivity settings in a cached storage service and use shallow equality helpers to bail early from write operations when the payload hasn't changed.
