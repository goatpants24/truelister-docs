## 2026-08-06 - [FlatList Grid Row-Based Virtualization Layout Offset]
**Learning:** In React Native, when `numColumns > 1` is configured on `FlatList`, the underlying `VirtualizedList` chunks individual items into row groups before computing the layout. Consequently, the `index` passed to `getItemLayout(data, index)` corresponds to the **row index** rather than the individual item index. Calculating the layout offset using an item-index formula (e.g. `Math.floor(index / numColumns)`) halves the actual vertical scroll offsets, resulting in misaligned layout, list jumping, and massive off-screen over-rendering.
**Action:** When using `getItemLayout` in multi-column `FlatList` components, always treat the `index` argument directly as the row index, and compute offsets as `rowHeight * index` to ensure correct virtualization.
