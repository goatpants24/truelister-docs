## 2026-08-05 - [Native Image Metadata and Regex Streamlining]
**Learning:** Retrieving image dimensions via `ImageManipulator.manipulateAsync` is a major performance bottleneck because it unnecessarily decodes and re-encodes the entire image file on disk (taking 400-800ms for large photos). Using React Native's `Image.getSize` fetches the width and height directly from the file's headers in ~1ms with zero re-encoding or storage overhead. Additionally, compiling a RegExp from an array that includes duplicate keywords slows down matching time due to backtracking overhead, and running split-map-join transformations on startup for known brands is redundant and alters casing for brands like "YSL" and "H&M".
**Action:** Always prefer lightweight metadata-only APIs (such as `Image.getSize` or `FileSystem.getInfoAsync`) to retrieve file dimensions/size before applying heavy manipulative operations. Ensure compiled regular expressions are stripped of redundant keywords and static startup lookup maps preserve pre-configured cased data directly.
