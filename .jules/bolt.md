## 2026-09-12 - [Horizontal Whitespace Restriction & Direct RegExp.exec]
**Learning:** Using generic whitespace character classes (`\s`) in multi-line OCR text parsing regexes can cause capturing groups to greedily cross newline boundaries (`\n`, `\r`), causing catastrophic regex backtracking and attribute line-bleed across lines. Restricting whitespace matching to horizontal tabs and spaces (`[\t ]`) restricts extraction strictly to the current line. Additionally, substituting `String.prototype.match` with direct `RegExp.exec` calls for non-global single-match patterns eliminates intermediate string match array allocations.
**Action:** When scanning multi-line OCR text or line-structured inputs, use horizontal whitespace character classes (`[\t ]`) instead of `\s` for capturing groups, and use `RegExp.exec` for single-match pattern searches.
