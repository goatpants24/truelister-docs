## 2026-08-06 - [FlatList getItemLayout with numColumns > 1 Offset Miscalculation]
**Learning:** When FlatList is configured with numColumns > 1 (grid view), VirtualizedList internally chunks the data array into rows. As a result, the index passed to the getItemLayout function corresponds to the row index, not the individual item index. Applying a column-division formula (like Math.floor(index / numColumns)) within getItemLayout is an anti-pattern that results in incorrect, overlapping, and truncated offsets. This breaks list virtualization, leading to massive over-rendering and scroll stutter.
**Action:** When using getItemLayout with a multi-column FlatList, always calculate the offset directly using the row index (offset = headerOffset + itemHeight * index) without division, and ensure the itemHeight represents the total height of a row container.
