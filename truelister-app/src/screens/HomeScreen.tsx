...
const GridItem = memo(({ item, thumbnailSize, onPress }: {
  ...
  return (
    ...
    >
      {item.photoUrl ? (
        <Image source={{ uri: item.photoUrl }} style={[styles.thumbnail, { width: size, height: size }]} resizeMode='cover' />
      ) : (
        <View style={[styles.thumbnail, { width: size, height: size, justifyContent: 'center', alignItems: 'center' }]}>
...
const ListItem = memo(({ item, onPress }: {
  ...
  return (
    <TouchableOpacity style={styles.listItem} onPress={() => onPress(item)}>
      {item.photoUrl && (
        <Image source={{ uri: item.photoUrl }} style={[styles.listThumbnail, { width: 64, height: 64 }]} resizeMode='cover' />
      )}
      <View style={styles.listTextContainer}>
        ...
        {item.price && <Text style={styles.listPrice}>${item.price}</Text>}
        {item.marketplace && <Text style={styles.listMarketplace}>{item.marketplace}</Text>}
      </View>
    </TouchableOpacity>
  );
});
...
export default function HomeScreen() {
  ...
  /** Bolt: Track previous data references to implement referential caching and SWR */
  ...
  const renderGridItem = useCallback(({ item }: { item: CatalogItem }) => {
    return <GridItem item={item} thumbnailSize={thumbnailSize} onPress={handleEditItem} />;
