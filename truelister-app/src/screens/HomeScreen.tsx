...
export default function HomeScreen() {
  ...
  const renderGridItem = useCallback(({ item }: { item: CatalogItem }) => (
    <GridItem item={item} thumbnailSize={thumbnailSize} onPress={handleEditItem} />
  ), [thumbnailSize, handleEditItem]);

  const renderListItem = useCallback(({ item }: { item: CatalogItem }) => (
    <ListItem item={item} onPress={handleEditItem} />
  ), [handleEditItem]);
...
function exportCSV(items: CatalogItem[]) {
  ...
  const rows = items.map((item) => [
    ...
    item.condition, item.fabricMaterial, item.color, item.price,
    item.marketplace, item.dateListed, item.notes, item.photoUrl,
  ]);
...
<head>
  ...
  <style>
    body { font-family: system-ui, sans-serif; background: #f9fafb; padding: 24px; margin: 0 auto; max-width: 1200px; }
    ...
    .no-image { height: 180px; background: #f3f4f6; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #6b7280; font-size: 14px; }
...
function showMarketplaceTemplates(items: CatalogItem[]) {
  Alert.alert('Marketplace Templates', 'Select a platform:', [
    { text: 'eBay', onPress: () => openMarketplaceTemplateModal(items, 'ebay') },
    { text: 'Mercari', onPress: () => openMarketplaceTemplateModal(items, 'mercari') },
    { text: 'Etsy / Facebook', onPress: () => openMarketplaceTemplateModal(items, 'etsy') },
    { text: 'Cancel', style: 'cancel' },
  ]);
}

function openMarketplaceTemplateModal(items: CatalogItem[], platform: string) {
  Alert.alert(
    `${platform.charAt(0).toUpperCase() + platform.slice(1)} Templates`,
    `Generated templates for ${items.length} items. Ready to copy.`,
    [{ text: 'OK' }]
  );
}
