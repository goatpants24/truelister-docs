...
/**
 * ⚡ BOLT PERFORMANCE OPTIMIZATION: Shallow Equality Helper
 * Faster than JSON.stringify for comparing state objects in hot paths.
 */
...
/**
 * ⚡ BOLT PERFORMANCE OPTIMIZATION: Upsert Logic
 * Saves a draft item locally. If an item with the same ID exists, it updates it.
 * Bails early if the content is identical, avoiding redundant AsyncStorage writes.
 */
export async function saveDraftItem(item: CatalogItem): Promise<void> {
  try {
    ...
    if (index !== -1) {
      if (shallowEqual(existing[index], item)) return; // No change
...
/**
 * ⚡ BOLT PERFORMANCE OPTIMIZATION: Early Exit
 * Skips AsyncStorage write if the item to remove doesn't exist.
 */
...
/**
 * ⚡ BOLT PERFORMANCE OPTIMIZATION: Upsert Logic & Memory Caching
 * Deduplicates pending uploads by itemNumber and fieldName.
 */
...
/**
 * ⚡ BOLT PERFORMANCE OPTIMIZATION: Memory Cache for Uploads
 */
...
export async function removePendingUpload(itemNumber: string): Promise<void> {
  try {
    const existing = await getPendingUploads();
    const index = existing.findIndex(u => u.itemNumber === itemNumber);
    if (index === -1) return;
