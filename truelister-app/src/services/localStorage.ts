 ...
 * Bolt: Faster than JSON.stringify for O(N) comparisons in data loops.
 */
...
export async function addPendingUpload(upload: PendingUpload): Promise<void> {
  try {
    ...
    if (index !== -1) {
      // Bolt: Use shallowEqual for precise deduplication
...
export async function saveSettings(settings: Partial<AppSettings>): Promise<void> {
  try {
    ...
    // Bolt: Skip AsyncStorage write if settings are unchanged
