...
export function parseTagText(rawText: string): Partial<CatalogItem> {
  ...
  if (brandMatch) {
    const matchedText = brandMatch[0].toLowerCase();
    // Use the correctly-cased original from our list
    const originalBrand = KNOWN_BRANDS.find(b => b.toLowerCase() === matchedText);
    if (originalBrand) {
      result.designerBrand = originalBrand;
    }
  }
  ...
  if (fabricMatches.length > 0) {
    ...
    FABRIC_REGEX.lastIndex = 0;
    const found = text.match(FABRIC_REGEX);
    if (found) {
      // Use Set for deduplication
      const unique = Array.from(new Set(found.map(f => f.charAt(0).toUpperCase() + f.slice(1).toLowerCase())));
      result.fabricMaterial = unique.join(', ');
    }
  }
  ...
  // ── Care Instructions ──
  CARE_REGEX.lastIndex = 0;
  const careFound = text.match(CARE_REGEX);
  if (careFound) {
    const uniqueCare = Array.from(new Set(careFound.map(c => c.toLowerCase())));
    const careNote = `Care: ${uniqueCare.join(', ')}`;
