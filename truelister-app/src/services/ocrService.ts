...
/**
 * Bolt: Restrict country extraction to horizontal whitespace ([\t ]) instead of \s
 * to keep matching on the current line. Prevents catastrophic regex backtracking across
 * multi-line OCR text (\n, \r) and eliminates line bleed into result.notes.
 */
...
export function parseTagText(rawText: string): Partial<CatalogItem> {
  ...
  // ── Brand Detection ──
  // Bolt: Use RegExp.exec instead of String.prototype.match for single-pattern scans to eliminate array allocations
