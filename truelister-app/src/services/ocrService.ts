...
/**
 * Bolt Performance Optimization: Restrict country of origin regex to horizontal whitespace.
 * Using [\t ] instead of \s prevents catastrophic backtracking across newlines (\n, \r)
 * during multi-line OCR tag scans, avoiding line-bleed into result.notes.
 */
...
export function parseTagText(rawText: string): Partial<CatalogItem> {
  ...
  // ── Brand Detection ──
  /**
   * Bolt Performance Optimization: Use RegExp.exec instead of String.prototype.match
   * Direct RegExp.exec avoids creating extra intermediate string match allocations.
   */
