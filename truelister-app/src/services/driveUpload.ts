...
export async function uploadToDrive(
  ...
  // Read both values from centralized storage cache — no restart needed after settings change
  const uploadEndpoint = await getAppsScriptUrl();
  const storedFolderId = await getDriveFolderId();
  const folderId = storedFolderId || GOOGLE_DRIVE_CONFIG.PHOTOS_FOLDER_ID || '';
