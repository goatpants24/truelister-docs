...
export async function fetchDropdowns(): Promise<DropdownOptions> {
  const now = Date.now();
  if (cachedDropdowns && (now - dropdownsCacheTimestamp < CACHE_TTL)) {
  ...
  try {
    ...
    cachedDropdowns = options;
    dropdownsCacheTimestamp = now;
    return options;
  } catch (error) {
