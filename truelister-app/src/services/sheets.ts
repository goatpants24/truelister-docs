...
// In-memory dropdown cache with 5-minute TTL to reduce redundant network requests
let cachedDropdowns: DropdownOptions | null = null;
let lastDropdownFetch: number = 0;
...
export async function fetchDropdowns(): Promise<DropdownOptions> {
  // Return cached dropdowns if they are fresh (within 5 minutes)
  const now = Date.now();
