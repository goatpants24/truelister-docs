...
import { getAppsScriptUrl, getSpreadsheetId } from './localStorage';
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        console.error('[Sheets] 404 Error: Please check your SPREADSHEET_ID in settings and ensure the sheet is "Published to the Web".');
      }
      return [];
    }
