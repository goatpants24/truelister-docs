...
// Performance: In-memory cache for sheet data to reduce redundant network requests
...
export async function fetchDropdowns(): Promise<DropdownOptions> {
  ...
  try {
    ...
    // Update cache
    dropdownsCache = { data, timestamp: Date.now() };
    return data;
  } catch (error) {
...
export async function appendItem(item: CatalogItem): Promise<boolean> {
  ...
  try {
    ...
    if (result.success === true) {
      // Invalidate inventory cache so home screen shows the new item
      inventoryCache = null;
      return true;
    }
    return false;
